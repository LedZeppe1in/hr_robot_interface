...
class PhraseDetector
{
    ...
    public static $YesPhraseSynonyms = ["да","разумеется", "безусловно", "конечно", "несомненно", "действительно",
        "само собой", "разумеется", "подлинно"];
    public static $NoPhraseSynonyms = ["нет","нет и нет","никогда", "увольте", "отсутствует", "никак нет", "нетушки",
        "нету", "нетути", "ничего подобного", "ни в коем случае", "ни под каким видом", "речи быть не может",
        "ни за что на свете", "ни за какие коврижки", "ни в коем разе", "ни за что", "не имеется", "не водится",
        "ни хрена", "ни в жизнь", "ни капли"];

    public static function GetDumpResult() {
        $result = array();
        $result[] = ["time" => 0.8, "val" => "Да", "text" => "Some word", "frame" => 0, "startFrame" => 0, "endFrame" => 0];
        $result[] = ["time" => 0.4, "val" => "Да", "text" => "one more word", "frame" => 1, "startFrame" => 0, "endFrame" => 2];
        $result[] = ["time" => 2.4, "val" => "Да", "text" => "final word", "frame" => 1, "startFrame" => 0, "endFrame" => 2];
        return result;
    }

    public static function GetPhraseEndTime($theTextData, $theStartIndex, $theEndIndex, $thePhrase)
    {
        if (isset($theTextData) && is_array($theTextData) && isset($theStartIndex) && isset($theEndIndex) &&
            isset($thePhrase) && isset($theTextData[$theStartIndex]) && isset($theTextData[$theEndIndex]) &&
            is_array($theTextData[$theStartIndex]) && is_array($theTextData[$theEndIndex]) &&
    ...
    {
        $curResult = [];
        if (isset($theFPS) && $theFPS > 0 && isset($theEndTime) && isset($theStartTime) && isset($theText)) {
            if (!isset($theTextType))
                $theTextType = "";
    ...
    public static function detectExactPhrase($theSource, $theIndex, $thePattern, $thePhrase, $theResult)
    {
        ...
        if (isset($theIndex) &&  isset($thePattern) && is_array($thePattern) && isset($theSource) &&
            is_array($theSource) && isset($theSource[$theIndex]) && is_array($theSource[$theIndex]) &&
            isset($theSource[$theIndex][0])) {
            ...
            if (count($curFilteredPattern) > 0)
                return PhraseDetector::detectExactPhrase(
                    $theSource,
                    $theIndex + 1,
                    $curFilteredPattern,
                    $thePhrase . " ",
        ...
        if (isset($theIndex) && isset($theSource) && is_array($theSource) && !isset($theSource[$theIndex])
            && isset($thePhrase)) {
    ...
    {
        if (isset($theTextData) && is_array($theTextData) && isset($theCheckList) && is_array($theCheckList) &&
            isset($theFPS) && isset($theAliasWord)) {
            $N = count($theTextData);
            if ($N > 0) {
                $startIndex = 0;
                if (isset($startFromTime))
                    $startIndex = TextFrequencyDetector::ResponseStartIndexInWords($theTextData, $startFromTime);
                $result = [];
                for ($i = $startIndex; $i < $N; $i++) {
                    $curOutput = PhraseDetector::detectExactPhrase(
                        $theTextData,
                        $i,
                        $theCheckList,
                        "",
                    ...
                    if (isset($curOutput) && $curOutput["found"]==true && isset($curOutput["endindex"]) &&
                        isset($curOutput["phrase"]) && isset($theTextData[$i][1])) {
                        $wordEndTime = PhraseDetector::GetPhraseEndTime($theTextData, $i,
                            $curOutput["endindex"], $curOutput["phrase"]);
                        $curResult = PhraseDetector::MakePhraseUnitSequence($theFPS, $curOutput["phrase"],
                            $theTextData[$i][1], $wordEndTime, $theAliasWord);
                        if (count($curResult))
                            $result[] = $curResult;
